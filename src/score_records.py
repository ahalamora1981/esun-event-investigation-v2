...
from .utils import load_config, setup_logger, dashscope_qwen_openai
from .records import (
    get_call_recording,
    get_email_records,
    get_qtrade_records,
...
def calculate_total_score(event: Event, record: dict) -> dict:
    ...
    total_score = (
        time_score * (WEIGHT_TIME / 100)
        + user_score * (WEIGHT_USER / 100)
        + content_score * (WEIGHT_CONTENT / 100)
...
def evaluate_record_risk(record: dict, records: list[dict]) -> dict:
    ...
    system_prompt_path = Path(__file__).parent / "prompts/evaluate_record_risk.md"
...
def print_records(new_event: Event, records: list[dict]):
    ...
    records_table.add_column("时间相关性", width=10)
    records_table.add_column("用户相关性", width=10)
    records_table.add_column("内容相关性", width=10)
    records_table.add_column("总体相关性", width=10, style="green bold")
    ...
    for i, record in enumerate(records):
        ...
        records_table.add_row(
            ...
            record["start_time"][-8:],
            record["channel"],
            USER_MAPPING.get(record["user_id"], record["user_id"]),
...
async def main():
    ...
    records = [
        ...
        if record["score"]["total_score"] >= THRESHOLD_TOTAL_SCORE
    ]
    ...
    # === 保存记录 ===
    output_fold = Path(__file__).parent / "output"
